...
def coulomb_descriptor_sorted_upper(atoms: Atoms, coords_in_bohr: bool = True) -> np.ndarray:
    M = get_coulomb_matrix(atoms, coords_in_bohr=coords_in_bohr)

    # keep only unique entries from symmetric matrix
    iu = np.triu_indices_from(M)
    v = M[iu].copy()

    # sort to make descriptor invariant to atom indexing
    v.sort()
    if SORT_DESCENDING:
        v = v[::-1]
...
def main():
    ...
    for idx, atoms in enumerate(read_multixyz_robust(XYZ_PATH)):
        d = coulomb_descriptor_sorted_upper(atoms, coords_in_bohr=COORDS_IN_BOHR)
        ...
        print(
            ...
            f"N_atoms = {len(atoms):2d} | descriptor_len = {len(d):3d}"
        )
    ...
    print(f"Max descriptor length: {X.shape[1]}")
